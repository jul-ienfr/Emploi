...
class ManagedBrowserClient:
    ...
    def _parse_timeout(self, timeout: float | int | None) -> float:
        raw: object = timeout if timeout is not None else os.environ.get("EMPLOI_MANAGED_BROWSER_TIMEOUT", "60")
        try:
    ...
    def _post_json(
        ...
        *,
        ...
        url = self.base_url + path
        payload = self._fetch_json(url, method="POST", body=body, action=action, site=site, profile=profile)
        return BrowserCommandResult(command=action, site=site, profile=profile, payload=payload)
    ...
    @with_retry(max_retries=3, base_delay=1.0, max_delay=10.0, retryable_exceptions=(URLError,))  # type: ignore[misc, arg-type]
