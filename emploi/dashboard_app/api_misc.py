"""Dashboard API — routes api misc bp (blueprint ``api_misc_bp``).
...
from emploi.dashboard_app.common import (
    _ensure_followed_companies,
    _ensure_user_profiles,
    _get_db,
)

api_misc_bp = Blueprint("api_misc_bp", __name__)
...
@api_misc_bp.route("/api/applications")
...
@api_misc_bp.route("/api/searches")
def api_searches():
    conn = _get_db()
    try:
...
@api_misc_bp.route("/api/cities/compare")
...
@api_misc_bp.route("/api/goals", methods=["GET", "POST"])
def api_goals():
    conn = _get_db()
    try:
        conn.execute(
...
@api_misc_bp.route("/api/search/semantic")
...
@api_misc_bp.route("/api/alerts", methods=["GET", "POST", "DELETE"])
def api_alerts():
    conn = _get_db()
    try:
        conn.execute(
...
@api_misc_bp.route("/api/webhooks", methods=["GET", "POST"])
def api_webhooks():
    conn = _get_db()
    try:
        conn.execute(
...
@api_misc_bp.route("/api/i18n/<lang>")
def api_i18n(lang):
    translations = {
        "fr": {
            "offers": "Offres",
...
@api_misc_bp.route("/api/import-sources")
...
@api_misc_bp.route("/api/search-history")
...
@api_misc_bp.route("/api/search-history", methods=["POST"])
...
@api_misc_bp.route("/api/profiles/<key>/default", methods=["POST"])
...
@api_misc_bp.route("/api/profiles/<key>/status")
...
@api_misc_bp.route("/api/searches/<int:search_id>/toggle", methods=["POST"])
...
@api_misc_bp.route("/api/bookmarks")
def api_bookmarks():
    conn = _get_db()
    try:
        rows = conn.execute(
...
@api_misc_bp.route("/api/chart-data")
def api_chart_data():
    conn = _get_db()
    try:
        # By source
        ...
        # By status
        by_status = dict(conn.execute("SELECT status, COUNT(*) FROM offers WHERE is_active = 1 GROUP BY 1").fetchall())
...
@api_misc_bp.route("/api/applications/<int:app_id>/status", methods=["POST"])
...
@api_misc_bp.route("/api/map-data")
def api_map_data():
    conn = _get_db()
    try:
        rows = conn.execute(
...
@api_misc_bp.route("/api/companies")
def api_companies():
    conn = _get_db()
    try:
        rows = conn.execute(
...
@api_misc_bp.route("/api/profiles/users", methods=["GET"])
...
@api_misc_bp.route("/api/profiles/users", methods=["POST"])
...
@api_misc_bp.route("/api/profiles/users/<int:profile_id>", methods=["GET"])
...
@api_misc_bp.route("/api/profiles/users/<int:profile_id>", methods=["PUT"])
...
@api_misc_bp.route("/api/profiles/users/<int:profile_id>", methods=["DELETE"])
...
@api_misc_bp.route("/api/analytics/conversion")
...
@api_misc_bp.route("/api/analytics/source-roi")
...
@api_misc_bp.route("/api/company/<name>/follow", methods=["POST"])
...
@api_misc_bp.route("/api/company/<name>/follow", methods=["DELETE"])
...
@api_misc_bp.route("/api/companies/followed")
...
@api_misc_bp.route("/api/import/clipboard", methods=["POST"])
...
@api_misc_bp.route("/api/import/url", methods=["POST"])
def api_import_url():
    data = request.get_json(force=True)
    ...
    for line in lines[:20]:
        if re.search(r"(?:entreprise|soci[ée]t[ée]|company)", line, re.IGNORECASE):
            company = re.sub(r"(?:entreprise|soci[ée]t[ée]|company)\s*[:\s]*", "", line, flags=re.IGNORECASE).strip()
...
@api_misc_bp.route("/api/import/text", methods=["POST"])
def api_import_text():
    data = request.get_json(force=True)
    text = data.get("text", "").strip()
    if not text:
    ...
    for line in lines:
        ...
        if re.match(r"^(?:titre|title|intitul[ée])\s*[:\s]+", lower):
            ...
            salary = re.sub(r"^(?:salaire|r[ée]mun[ée]ration|salary)\s*[:\s]+", "", line, flags=re.IGNORECASE).strip()
...
@api_misc_bp.route("/api/profile/skills", methods=["POST"])
...
@api_misc_bp.route("/api/skill-match/<int:offer_id>")
...
@api_misc_bp.route("/api/salary-analysis")
...
@api_misc_bp.route("/api/application/<int:app_id>/timeline")
...
@api_misc_bp.route("/api/application/<int:app_id>/followup", methods=["POST"])
...
@api_misc_bp.route("/api/analytics/response-rate")
def api_response_rate():
    conn = _get_db()
    try:
        total = conn.execute("SELECT COUNT(*) FROM applications").fetchone()[0]
        responded = conn.execute("SELECT COUNT(*) FROM applications WHERE status NOT IN ('draft', 'sent')").fetchone()[
            0
        ]
...
@api_misc_bp.route("/api/analytics/weekly")
...
@api_misc_bp.route("/api/reminders", methods=["GET"])
...
@api_misc_bp.route("/api/reminders", methods=["POST"])
...
@api_misc_bp.route("/api/reminders/<int:reminder_id>", methods=["GET"])
...
@api_misc_bp.route("/api/reminders/<int:reminder_id>", methods=["PUT"])
...
@api_misc_bp.route("/api/reminders/<int:reminder_id>", methods=["DELETE"])
